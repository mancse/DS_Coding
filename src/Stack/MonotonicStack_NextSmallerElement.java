...
public class NextSmallerElement {
	public static void main(String args[]){
		...
		int res[]=getNextSmallerElement(nums);
		for (int i: res){
			System.out.print(" "+i);
		}
	}
	...
	/**
	 *    Idea: Stack top maintins the arry element such that stack.peek() less than array current 
	 ...
	 *      -Check if stack is empty then add -1 in result array at current index else add stack top 
	 *    index - current index in the answer array.
	 *      -Push the current index into the index.
	 *    
	 */
	private static int[] getNextSmallerElement(int nums[]){
